...
// 📈━━━━━━━━━━━━━━━━━━━ [ © NeoDev ] ━━━━━━━━━━━━━━━━━━━📉//
const fs = require('fs');
const koboyDBPath = './SadSystem/SadDatabase/Game/koboy.json';
...
module.exports = async function koboyHandler(m, command, args, SadReply, db) {
...
const shop = {
  ...
  switch (sub) {
    case 'daftar':
      SadReply('🤠 Kamu sudah terdaftar sebagai koboy!');
      break;
...
case 'status':
  ...
  userData.misi.progress += 1;
      ...
      SadReply(`💼 Kamu bekerja dan mendapatkan 💰 ${hasilKerja}`);
      break;

case 'tembak': {
      if (!m.mentionedJid[0]) return SadReply('🚨 Tag pemain yang ingin ditembak!');
      const target = m.mentionedJid[0];
      if (!koboyData[target]) return SadReply('❌ Target belum daftar sebagai koboy.');

      if (!userData.inventori.includes('peluru')) return SadReply('🔫 Kamu kehabisan peluru!');
      ...
      if (menang) {
        ...
        SadReply(`🔫 Kamu menang duel!\n- Damage: ${finalDamage}\n- Ambil 💰 ${curian}\n- EXP +${finalDamage}`);
      } else {
        userData.uang = Math.max(0, userData.uang - 100);
        SadReply(`💥 Kamu kalah duel! Kehilangan 💰 100 dan 1 peluru.`);
      }
      break;
    }
...
case 'beli':
  ...
  if (!shop[itemBeli] || !stokHariIni.includes(itemBeli)) {
    return SadReply('❌ Item tidak tersedia di toko.\nTersedia: ' + stokHariIni.join(', '));
  }

  const harga = shop[itemBeli];
  if (userData.uang < harga) return SadReply('💸 Uang kamu tidak cukup!');

  // ✅ Tambahkan ini sebelum item masuk inventori
  if (userData.inventori.length >= userData.kapasitas)
    return SadReply('🎒 Tas kamu penuh! Tingkatkan kapasitas atau jual item.');
  ...
  if (itemBeli === 'tas besar') {
    ...
    SadReply(`🎒 Kamu membeli *tas besar*! Kapasitas inventori bertambah jadi ${userData.kapasitas} slot.`);
  } else {
    SadReply(`🛒 Kamu membeli *${itemBeli}* seharga 💰 ${harga}`);
  }
  ...
  break;
    ...
    case 'jual':
      ...
      if (!userData.inventori.includes(itemJual)) return SadReply('❌ Kamu tidak punya item itu.');
      ...
      SadReply(`💵 Kamu menjual *${itemJual}* seharga 💰 ${jualHarga}`);
      break;
    ...
    case 'wanted':
      if (userData.role.toLowerCase() !== 'bandit') return SadReply('❌ Hanya Bandit bisa jadi Wanted.');
      userData.wanted = true;
      SadReply(`🚨 Kamu sekarang WANTED! Hati-hati jika bertemu Sheriff!`);
      break;
    ...
    case 'topkoboy':
      const top = Object.entries(koboyData)
        ...
        .join('\n') || 'Belum ada data';
      SadReply(`🏆 Top 5 Koboy Kaya:\n\n${top}`);
      break;

    case 'peran':
      SadReply('👤 Peran kamu saat ini: ' + userData.role);
      break;
      
    case 'tangkap':
      if (userData.role.toLowerCase() !== 'sheriff') return SadReply('❌ Hanya Sheriff yang bisa menangkap!');
      if (!m.mentionedJid[0]) return SadReply('🚨 Tag pemain yang ingin ditangkap!');
      const targetTangkap = m.mentionedJid[0];
      if (!koboyData[targetTangkap]) return SadReply('❌ Target belum terdaftar.');
      if (!koboyData[targetTangkap].wanted) return SadReply('✅ Target bukan bandit yang dicari.');

      // Hadiah penangkapan
      userData.uang += 300;
      koboyData[targetTangkap].wanted = false;
      SadReply(`🪤 Kamu menangkap ${koboyData[targetTangkap].nama}! Dapat 💰 300`);
      break;

    case 'lepas':
      if (userData.role.toLowerCase() !== 'sheriff') return SadReply('❌ Hanya Sheriff yang bisa melepas tahanan!');
      if (!m.mentionedJid[0]) return SadReply('🚨 Tag pemain yang ingin dilepas!');
      const targetLepas = m.mentionedJid[0];
      if (!koboyData[targetLepas]) return SadReply('❌ Target belum terdaftar.');
      koboyData[targetLepas].wanted = false;
      SadReply(`🔓 Kamu melepaskan ${koboyData[targetLepas].nama} dari status wanted.`);
      break;

    case 'hukum':
      if (userData.role.toLowerCase() !== 'sheriff') return SadReply('❌ Hanya Sheriff yang bisa menghukum!');
      if (!m.mentionedJid[0]) return SadReply('🚨 Tag pemain yang ingin dihukum!');
      const targetHukum = m.mentionedJid[0];
      if (!koboyData[targetHukum]) return SadReply('❌ Target belum terdaftar.');
      if (!koboyData[targetHukum].wanted) return SadReply('✅ Target bukan bandit yang dicari.');
      ...
      SadReply(`⚖️ ${koboyData[targetHukum].nama} dihukum! Kehilangan 💰200 dan EXP 20`);
      break;
...
case 'misi':
      ...
      if (!userData.misi || userData.misi.terakhir !== now) {
        ...
        return SadReply(`🎯 Misi harian kamu:\n${misi.teks}\n\nGunakan kembali *.koboy misi* untuk cek progres.`);
      }

      if (userData.misi.status === 'selesai') {
        return SadReply('✅ Kamu sudah menyelesaikan misi hari ini.');
      }
      ...
      if (userData.misi.progress >= userData.misi.target) {
        ...
        return SadReply(`📜 Misi selesai!\n+💰 ${hadiahUang}\n+📘 EXP ${hadiahExp}`);
      }

      return SadReply(`📊 Progres Misi:\n${userData.misi.teks}\n${userData.misi.progress}/${userData.misi.target}`);
      
       
      case 'kirim':
      const itemKirim = args[1]?.toLowerCase();
      const targetKirim = m.mentionedJid[0];
      if (!itemKirim || !targetKirim) return SadReply('🔁 Format: .koboy kirim [item] @user');

      if (!userData.inventori.includes(itemKirim)) return SadReply('❌ Kamu tidak punya item itu!');
      if (!koboyData[targetKirim]) return SadReply('❌ Target belum terdaftar.');
...
if (userData.misi?.jenis === 'kirim' && userData.misi.status === 'proses') {
  userData.misi.progress += 1;
      ...
      SadReply(`📦 Item *${itemKirim}* berhasil dikirim ke ${koboyData[targetKirim].nama}`);
      break;
...
case 'kota':
      ...
      case 'toko': {
        ...
        case 'penjara':
          ...
          SadReply(`🧾 *DAFTAR BURONAN*\n\n${wantedList}`);
          break;
        case 'sheriff':
          SadReply('👮 *KANTOR SHERIFF*\nKamu bisa gunakan .koboy tangkap atau .koboy hukum untuk menangani bandit!');
          break;
        default:
          SadReply('❌ Tempat tidak ditemukan. Gunakan: .koboy kota');
      }
      break;      
      
case 'setor':
      const jumlahSetor = parseInt(args[1]);
      if (!jumlahSetor || isNaN(jumlahSetor)) return SadReply('💰 Masukkan jumlah uang yang ingin disetor!\nContoh: .koboy setor 100');
      if (jumlahSetor > userData.uang) return SadReply('💸 Uang kamu tidak cukup!');
      userData.uang -= jumlahSetor;
      userData.bank += jumlahSetor;
      SadReply(`🏦 Kamu menyetor 💰 ${jumlahSetor} ke bank.`);
      break;
      
            
case 'tarik':
      const jumlahTarik = parseInt(args[1]);
      if (!jumlahTarik || isNaN(jumlahTarik)) return SadReply('💰 Masukkan jumlah uang yang ingin ditarik!\nContoh: .koboy tarik 100');
      if (jumlahTarik > userData.bank) return SadReply('🏦 Uang di bank tidak cukup!');
      userData.bank -= jumlahTarik;
      userData.uang += jumlahTarik;
      SadReply(`💸 Kamu menarik 💰 ${jumlahTarik} dari bank.`);
      break;

case 'cekstok': {
  const stokHariIni = buatTokoHarian();
  SadReply('🛒 Stok toko hari ini:\n' + stokHariIni.join(', '));
  break;
}

case 'rampok':
  if (userData.role.toLowerCase() !== 'bandit') return SadReply('❌ Hanya Bandit yang bisa merampok!');
  if (!m.mentionedJid[0]) return SadReply('🚨 Tag pemain yang ingin dirampok!');
  const targetRampok = m.mentionedJid[0];
  if (!koboyData[targetRampok]) return SadReply('❌ Target belum terdaftar.');

  const targetDataRampok = koboyData[targetRampok];
  if (targetDataRampok.uang < 100) return SadReply('💸 Target terlalu miskin, tidak bisa dirampok.');
  ...
  if (sukses) {
    ...
    userData.exp += 10;
    SadReply(`🏴‍☠️ Kamu berhasil merampok ${targetDataRampok.nama} dan mendapatkan 💰 ${hasilRampok}!`);
  } else {
    userData.uang = Math.max(0, userData.uang - 150);
    userData.wanted = true;
    SadReply('💥 Rampokan gagal! Kamu ketahuan dan kehilangan 💰150.\n🚨 Sekarang kamu jadi *WANTED*!');
  }
  break;

case 'equip': {
      const itemEquip = args.slice(1).join(' ').toLowerCase();
      if (!itemEquip) return SadReply('⚙️ Format: .koboy equip [item]');
      if (!userData.inventori.includes(itemEquip)) return SadReply('❌ Kamu tidak memiliki item tersebut!');

      if (itemEquip.includes('pistol') || itemEquip.includes('shotgun')) {
        userData.senjata = itemEquip;
        return SadReply(`🔫 Senjata kamu sekarang: *${itemEquip}*`);
      }
     if (itemEquip === 'armor') {
  return SadReply('🛡️ Armor tidak perlu di-equip, otomatis aktif jika kamu memilikinya.');
}
      return SadReply('❌ Item tidak bisa dipasang/equip.');
    }
  break;

case 'daily':
  const today = new Date().toDateString();
  if (userData.daily === today) return SadReply('📆 Kamu sudah klaim bonus hari ini!');
  ...
  SadReply(`🎁 Bonus harian diklaim!\n+💰 ${bonusUang} uang\n+📘 ${bonusExp} EXP`);
  break;
  
case 'minum':
  const hargaMinum = 50;
  if (userData.uang < hargaMinum) return SadReply('🍺 Uang kamu kurang! Minuman harganya 💰 50');

  userData.uang -= hargaMinum;
  const bonus = Math.floor(Math.random() * 10) + 5;
  userData.exp += bonus;

  SadReply(`🍻 Kamu menikmati minuman di saloon.\n+📘 ${bonus} EXP, -💰 50`);
  break;
...
case 'kerja_berani':
  ...
  if (suksess) {
    ...
    userData.uang += hasil;
    userData.exp += 10;
    SadReply(`💼 Kerja berani sukses! Kamu dapat 💰 ${hasil} dan 📘 10 EXP`);
  } else {
    const rugi = Math.floor(Math.random() * 100) + 50;
    userData.uang = Math.max(0, userData.uang - rugi);
    SadReply(`💥 Kerja gagal total! Kamu rugi 💰 ${rugi}`);
  }
  break;

case 'invest':
  const jumlahInvest = parseInt(args[1]);
  if (!jumlahInvest || isNaN(jumlahInvest)) return SadReply('💼 Masukkan jumlah uang untuk investasi!\nContoh: .koboy invest 100');
  if (jumlahInvest > userData.uang) return SadReply('💸 Uang kamu tidak cukup!');
  ...
  SadReply(`🏦 Kamu menginvestasikan 💰 ${jumlahInvest}.\nGunakan *.koboy cair* setelah 24 jam untuk ambil hasil.`);
  break;

case 'cair':
  if (!userData.investasi || userData.investasi.jumlah <= 0) return SadReply('❌ Kamu tidak punya investasi aktif.');
...
case 'tebak':
  ...
  if (angkaTebakan === angkaBenar) {
    ...
    userData.exp += 10;
    SadReply(`🎯 Benar! Angka: ${angkaBenar}\n+💰 ${hadiah} uang\n+📘 10 EXP`);
  } else {
    const rugi = 50;
    userData.uang = Math.max(0, userData.uang - rugi);
    SadReply(`❌ Salah! Angka yang benar: ${angkaBenar}\n-💰 ${rugi}`);
  }
  break;
...
case 'toplevel':
  const topLevel = Object.entries(koboyData)
    ...
    .join('\n') || 'Belum ada data.';
  SadReply(`🏆 *Top Level Koboy*\n\n${topLevel}`);
  break;     

case 'roll': {
  const target = m.mentionedJid[0];
  if (!target) return SadReply('🎲 Tag pemain yang ingin diajak adu dadu!');
  if (!koboyData[target]) return SadReply('❌ Target belum terdaftar sebagai koboy.');
...
case 'jualke': {
  ...
  if (!item || !harga || !targetJual)
    return SadReply(`📦 Format: .koboy jualke [item] [harga] @user\nContoh: .koboy jualke peluru 150 @target`);

  if (!userData.inventori.includes(item))
    return SadReply(`❌ Kamu tidak punya item *${item}*`);

  if (!koboyData[targetJual])
    return SadReply(`🚫 Target belum terdaftar.`);
  ...
  SadReply(`📬 Kamu menawarkan *${item}* seharga 💰${harga} ke ${koboyData[targetJual].nama}.\nMereka bisa membelinya dengan .koboy beliitem ${item} @${user.split('@')[0]}`);
  break;
}

case 'pasarku': {
  if (!userData.pasar || userData.pasar.length === 0) {
    return SadReply('📭 Tidak ada tawaran jual item ke kamu saat ini.');
  }
...
case 'beliitem': {
  ...
  if (!itemBeli || !penjual) return SadReply('🛒 Format: .koboy beliitem [item] @penjual');

  if (!koboyData[penjual]) return SadReply('❌ Penjual tidak terdaftar.');
  const penjualData = koboyData[penjual];

  const tawaran = penjualData?.tawaranku?.find(t => t.item === itemBeli && t.kepada === user);
  if (!tawaran) return SadReply('❌ Penjual tidak menawarkan item itu ke kamu.');

  if (userData.uang < tawaran.harga) return SadReply('💸 Uang kamu tidak cukup untuk membeli item ini.');
  ...
  SadReply(`✅ Kamu membeli *${itemBeli}* dari ${penjualData.nama} seharga 💰 ${tawaran.harga}`);
  break;
}

case 'bataljual': {
  const itemBatal = args.slice(1).join(' ').toLowerCase();
  if (!itemBatal) return SadReply('⚠️ Format: .koboy bataljual [item]');

  const daftar = userData.tawaranku || [];
  const tawaran = daftar.find(t => t.item === itemBatal);

  if (!tawaran) return SadReply('❌ Kamu tidak sedang menawarkan item itu.');
  ...
  SadReply(`✅ Penawaran *${itemBatal}* telah dibatalkan.`);
  break;
}

    default:
      SadReply(`⚠️ Sub-command tidak ditemukan.\nContoh: .koboy kerja, .koboy beli pistol, .koboy wanted`);
  }
