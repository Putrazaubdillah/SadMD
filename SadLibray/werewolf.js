...
async function pagi(Sad, x, data) {
    ...
    return await Sad.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(Sad, x, data) {
    let row = [];
    let ment = [];
...
async function malam(Sad, x, data) {
...
async function skill(Sad, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await Sad.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, Sad, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await Sad.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(Sad, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(Sad, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Sad, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(Sad, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Sad, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(Sad, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Sad, data);
            break;
        } else {
            await skill(Sad, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, Sad, data);
}

async function run_vote(Sad, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Sad, data);
            break;
...
async function run_malam(Sad, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Sad, data);
            break;
...
async function run_pagi(Sad, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Sad, data);
            break;
